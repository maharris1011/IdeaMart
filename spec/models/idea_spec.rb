...
describe Idea do
	before(:all) do
		@user = User.new(:name => 'Mark E. Mark', :email => 'mharris@manta.com', :password => 'password', :launchpad=>false)
		@lp_user = User.new(:name => 'Launchpad User', :email => 'mharris@manta.com', :password => 'password', :launchpad=> true)
		# @user.save
		# @lp_user.save
	end
	...
	it 'moves to proposed if has > 5 votes, a concept, a sponsor, customers, a problem, and a solution' do
		...
		(0..5).each do |i|
			u = User.new(:name => "User #{i}", :password => 'password', :email => "m#{i}@manta.com", :launchpad => false)
